<script setup>
import { ref } from 'vue'

defineProps({
  tituloPagina: {
    type: String,
    required: true
  },
  proyecto: {
    type: Object,
    required: true
  },
  slides: {
    type: Array,
    required: true
  }
})

const carousel = ref(0)
</script>

<template>
  <v-container class="pb-8" style="padding-top: 55px !important">
    <v-row class="mb-6 mt-16">
      <v-col cols="12" class="text-center">
        <h1 class="text-h3 mb-4" style="color: #04323a; font-weight: 500; letter-spacing: -0.5px">
          {{ tituloPagina }}
        </h1>
        <div class="title-divider mb-6"></div>
        <h2 class="text-h5 mb-4" style="color: #04323a">{{ proyecto.titulo }}</h2>
        <p v-if="proyecto.anio" class="text-caption mb-0" style="color: #666">{{ proyecto.anio }}</p>
      </v-col>
    </v-row>

    <v-row v-if="slides.length" class="mb-6">
      <v-col cols="12" md="8" class="mx-auto">
        <v-card elevation="0" style="border: 1px solid #e0e0e0">
          <v-carousel
            v-model="carousel"
            :show-arrows="slides.length > 1"
            hide-delimiter-background
            delimiter-icon="mdi-circle"
            class="carousel"
            height="400"
            :cycle="slides.length > 1"
            :interval="5000"
          >
            <v-carousel-item
              v-for="(slide, i) in slides"
              :key="i"
              :src="slide.src"
              :alt="slide.alt"
              class="carousel-item"
            />
          </v-carousel>
        </v-card>
      </v-col>
    </v-row>

    <v-row class="mb-6">
      <v-col cols="12" md="8" class="mx-auto">
        <p class="text-subtitle-1" style="color: #666">{{ proyecto.descripcion }}</p>
      </v-col>
    </v-row>

    <v-row>
      <v-col cols="12" md="6">
        <v-card elevation="0" class="mb-6" style="border: 1px solid #e0e0e0">
          <v-card-text>
            <h3 class="text-h6 mb-4" style="color: #04323a">Sobre el Proyecto</h3>
            <p class="text-body-2 mb-4" style="color: #333">{{ proyecto.detalles.contexto }}</p>
            <p class="text-body-2" style="color: #333">{{ proyecto.detalles.conclusion }}</p>
          </v-card-text>
        </v-card>
      </v-col>

      <v-col cols="12" md="6">
        <v-card elevation="0" class="mb-6" style="border: 1px solid #e0e0e0">
          <v-card-text>
            <h3 class="text-h6 mb-4" style="color: #04323a">Características Principales</h3>
            <v-list class="pa-0">
              <v-list-item
                v-for="(caracteristica, index) in proyecto.caracteristicas"
                :key="index"
                class="px-0"
              >
                <template v-slot:prepend>
                  <v-icon color="#04323a" size="small">mdi-check</v-icon>
                </template>
                <v-list-item-title style="color: #333">{{ caracteristica }}</v-list-item-title>
              </v-list-item>
            </v-list>
          </v-card-text>
        </v-card>
      </v-col>
    </v-row>

    <v-row class="mt-6">
      <v-col cols="12">
        <v-card elevation="0" style="border: 1px solid #e0e0e0">
          <v-card-text>
            <h3 class="text-h6 mb-4 text-center" style="color: #04323a">Tecnologías</h3>
            <v-chip-group class="d-flex justify-center flex-wrap">
              <v-chip
                v-for="tech in proyecto.tecnologias"
                :key="tech"
                variant="outlined"
                class="mr-2 mb-2"
                style="border-color: #04323a; color: #04323a"
              >
                {{ tech }}
              </v-chip>
            </v-chip-group>
          </v-card-text>
        </v-card>
      </v-col>
    </v-row>
  </v-container>
</template>

<style scoped>
.v-card {
  background-color: white;
  transition: transform 0.2s;
}

.v-card:hover {
  transform: translateY(-2px);
}

.v-list-item {
  min-height: 40px;
}

.text-body-2 {
  line-height: 1.6;
}

.title-divider {
  width: 60px;
  height: 3px;
  background-color: #04323a;
  margin: 0 auto;
}

.carousel {
  border-radius: 4px;
  overflow: hidden;
}

.carousel-item {
  display: flex;
  align-items: center;
  justify-content: center;
  background-color: #f8f7f2;
}

@media (max-width: 960px) {
  .carousel-item :deep(img) {
    max-height: 300px;
  }
}
</style>
